...
__global__ void kernelRenderCirclesPerPixel(int numCircles)
{
    ...
    {
        float3 pos = *(float3 *)(&cuConstRendererParams.position[3 * c]);
...
__global__ void kernelCountTileCircles(
    int tilesPerRow,
    int tilesPerCol,
    int *tileCircleCount)
{
    ...
    int ty = tileIdx / tilesPerRow;
    int tx = tileIdx % tilesPerRow;

    // Convert tile coordinates to normalized screen coordinates [0, 1]
    float tileLeft = (float)tx / tilesPerRow;
    float tileRight = (float)(tx + 1) / tilesPerRow;
    float tileBottom = (float)ty / tilesPerCol;
    float tileTop = (float)(ty + 1) / tilesPerCol;
    ...
    for (int c = 0; c < cuConstRendererParams.numCircles; c++)
    {
        ...
        // Test if circle intersects this tile
        if (circleInBox(pos.x, pos.y, rad, tileLeft, tileRight, tileTop, tileBottom))
        {
            count++;
        }
    }

    tileCircleCount[tileIdx] = count;
}

// Each thread processes one tile.
__global__ void kernelFillTileCircles(
    int tilesPerRow,
    int tilesPerCol,
    int *tileCircleCount,
    ...
    int *tileCircleIndices)
{
    ...
    int offset = tileCircleOffsets[tileIdx];
    int writeIdx = 0;

    // Write all circles that intersect this tile
    for (int c = 0; c < cuConstRendererParams.numCircles; c++)
    {
        float3 pos = *(float3 *)(&cuConstRendererParams.position[3 * c]);
        float rad = cuConstRendererParams.radius[c];
        ...
        if (circleInBox(pos.x, pos.y, rad, tileLeft, tileRight, tileTop, tileBottom))
        {
            tileCircleIndices[offset + writeIdx] = c;
            writeIdx++;
        }
    }
}
...
CudaRenderer::CudaRenderer()
{
    ...
    // Initialize tile mapping structures
    tilesPerRow = 0;
    tilesPerCol = 0;
    numTiles = 0;
    cudaTileCircleCount = NULL;
    cudaTileCircleIndices = NULL;
    cudaTileCircleOffsets = NULL;
}
...
CudaRenderer::~CudaRenderer()
{
    ...
    // Free tile mapping DS
    if (cudaTileCircleCount)
    {
        cudaFree(cudaTileCircleCount);
        cudaFree(cudaTileCircleIndices);
        cudaFree(cudaTileCircleOffsets);
    }
}
...
void CudaRenderer::buildTileMapping()
{
    if (numTiles <= 0)
        return;

    // Allocating device memory for tile mapping DS
    if (cudaTileCircleCount)
    {
        cudaFree(cudaTileCircleCount);
        cudaFree(cudaTileCircleIndices);
    ...
    kernelCountTileCircles<<<gridSize, blockSize>>>(tilesPerRow, tilesPerCol, cudaTileCircleCount);
    cudaDeviceSynchronize();

    int *hostCounts = new int[numTiles];
    cudaMemcpy(hostCounts, cudaTileCircleCount, sizeof(int) * numTiles, cudaMemcpyDeviceToHost);

    int *hostOffsets = new int[numTiles];
    hostOffsets[0] = 0;
    ...
    // Filling the circle indices for each tile
    kernelFillTileCircles<<<gridSize, blockSize>>>(tilesPerRow, tilesPerCol,
                                                   cudaTileCircleCount,
    ...
    printf("Tile mapping built: %d tiles (%d x %d), total circles per tile: %d\n",
...
void CudaRenderer::allocOutputImage(int width, int height)
{
    ...
    tilesPerRow = (width + TILE_SIZE - 1) / TILE_SIZE;
    tilesPerCol = (height + TILE_SIZE - 1) / TILE_SIZE;
